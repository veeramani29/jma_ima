...
//Get post main category data
router.get('/getPostMainCategory', (req, res, err) => {
	con.query('SELECT * FROM ' + postCategory_table, (err, rows) => {
		if (!err) {
			// console.log(rows);
			res.send(rows);
		} else {
...
//Get post sub category data
router.post('/getPostSubCategory', (req, res, err) => {
	con.query('SELECT post_category_parent_id,post_category_id,post_category_name FROM ' + postCategory_table + ' WHERE post_category_parent_id =' + req.body.id, (err, rows, fields) => {
		if (!err) {
			// console.log(rows);
			res.send(rows);
		} else {
...
//Get post grand sub category data
router.post('/getPostGrandSubCategory', (req, res, err) => {
	con.query('SELECT post_category_parent_id,post_category_id,post_category_name FROM ' + postCategory_table + ' WHERE post_category_parent_id =' + req.body.id, (err, rows, fields) => {
		if (!err) {
			// console.log(rows);
			res.send(rows);
...
//Insert post data
router.post('/addPost', [check('post_category_id').isNumeric(), check('post_title').isLength({
	min: 2
}), check('post_cms_small').isLength({
	min: 5
}), check('post_cms').isLength({
	min: 10
})], (req, res, err) => {
...
//Update post data
router.post('/updatePost', [check('post_category_id').isNumeric(), check('post_title').isLength({
	min: 2
}), check('post_cms_small').isLength({
	min: 5
}), check('post_cms').isLength({
...
router.post('/archivePost', [check('post_title').isLength({
	min: 2
}), check('post_cms').isLength({
	min: 10
})], (req, res, err) => {
	...
	con.query("SELECT id,SUBSTRING_INDEX(post_datetime,' ',1) AS column_date FROM " + archive_table + " WHERE  post_title='" + req.body.post_title + "' HAVING DATEDIFF('" + dateTime + "',column_date) < '15'", (err, rows, fields) => {
		if (rows.length != 0) {
			con.query("SELECT id,post_cms FROM " + archive_table + " WHERE  post_title= '" + req.body.post_title + "' ORDER BY id DESC LIMIT 1", (err, postCms, fields) => {
				if (postCms[0].post_cms.replace(/<\/?[^>]+(>|$)/g, "") != req.body.post_cms.replace(/<\/?[^>]+(>|$)/g, "")) {
...
router.get('/send-email-alert/:id/:post_type', (req, res, err) => {
	...
	if (env('APP_ENV') == "development" || env('APP_ENV') == "test") {
		var list_Objects = [{
				"name": "Veera",
				"address": "veeramani.kamaraj@japanmacroadvisors.com"
			},
			{
				"name": "Sadia",
				"address": "sadia.siddiqa@japanmacroadvisors.com"
			},
			{
				"name": "Vinesh",
				"address": "vigneswaran.sekar@japanmacroadvisors.com"
			}
		]
	...
	Mailgun_Lib.fetchPostData(Postid, function (err, rows) {
		...
		var link = env('APP_URL') + '/page/category/' + postURL;
		console.log(link);
	...
	Mailgun_Lib.changeStatus(Postid, Poststatus, function (err, affectedRows) {
		if (affectedRows) {
			...
			Mailgun_Lib.fetchPostData(Postid, function (err, rows) {
				...
				if (Mailgun_Lib.checkAlreadysent(Posttype, post_publish_status, post_mail_notification)) {
					console.log('checkAlreadysent')
					...
					var link = env('APP_URL') + '/reports/view/' + postURL;
					console.log(link);
					...
					res_send.message = 'Published and sent emails also!';
					res.send((res_send));
					...
					res_send.message = 'Already published and sent emails also! Do you wnat to send again';
					res.send((res_send));
			...
			res_send.message = 'Something troubling changing status of post publish status';
			res.send((res_send));
