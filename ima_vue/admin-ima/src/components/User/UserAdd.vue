...
export default {
  ...
  methods: {
    ...
    submit() {
      if (this.$refs.form.validate()) {
        if (typeof this.country != "number") this.country = "";
        if (typeof this.company != "number") this.company = "";
        if (typeof this.userType != "number") this.userType = "";
        if (typeof this.userStatus != "number") this.userStatus = "";
        if (typeof this.emailVerified == "object") this.emailVerified = "";
        if (typeof this.postAlert == "object") this.postAlert = "";
        // Math.floor(date.getTime()/1000)
        var currendDate = new Date().toISOString().slice(0, 10);
        var registerDate = new Date(currendDate).getTime() / 1000;
        var expiryDate = "";
        if (this.date == "") {
          expiryDate = 0;
        } else {
        ...
        // var test =new Date (this.date);
        // console.log(test.getFullYear() + "-" + ("0"+(test.getMonth() + 1)).slice(-2) + "-"+("0"+test.getDate()).slice(-2))
        this.insert = this.axios
          .post(this.nodejsServer + "user/insertUser", userData)
          .then(response => {
            if (!response.data.err_code) {
