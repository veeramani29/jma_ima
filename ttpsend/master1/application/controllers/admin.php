...
class Admin extends CI_Controller {
		...
		define('ADMIN_LOGO', site_url().'assets/uploads/admins_logo/');
...
if ($this->form_validation->run() == TRUE)
		{ 
			...
			$old_file = $this->input->post('hdnoldfile');
				...
				$config['upload_path']='./assets/uploads/admins_logo';
...
if($this->input->post('change')!=null){
$this->form_validation->set_rules('new_password', 'Current Password', 'trim|matches[Oldpassword]|xss_clean');
$this->form_validation->set_rules('password', 'New Password', 'trim|required|matches[passconf]|xss_clean');
$this->form_validation->set_rules('passconf', 'Password Confirmation', 'trim|required|xss_clean');
	...
	$this->form_validation->set_rules('notes', 'Notes', 'trim|required|xss_clean');
			...
			$get_all_admins=$this->Admin_Model->get_edit_admins($id);
			$data['admin_details']=$get_all_admins;
