...
{	
	...
	public function get_Company($id)
	{
		...
		$this->db->where('id',$id);
		$get_results = $this->db->get('companies_details')->row_array();
	...
	public function get_Tariff($id)
	{
		...
		$this->db->where('id',$id);
		$get_results = $this->db->get('tariffs_details')->row_array();
		
		
		return $get_results;
	}

  public function get_companiename($code){
        $this->db->select('name');
        $this->db->where('id', $code);
        $data = $this->db->get('companies_details')->row();
	...
	public function get_allCompanies()
	{
			...
			$get_results = $this->db->get('companies_details');
	...
	public function get_allVehicle_Categories()
	{

			$mail_alldetails=array();
			$this->db->group_by('cate_name');
	...
	public function get_allTariffs()
	{

			$mail_alldetails=array();
			$get_results = $this->db->get('tariffs_details');
			$mail_alldetails['num_rows']=$get_results->num_rows;
			if($get_results->num_rows>0){
			$mail_alldetails['results']=$get_results->result_array();
			}
