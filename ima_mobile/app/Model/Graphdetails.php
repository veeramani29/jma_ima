...
class Graphdetails extends Model {
	...
	public function getMapDetailsForJsonDataAll($gids) {
		...
		$response = array();
		$sql = "SELECT mv.x_value,mv.value as value FROM map_values AS mv WHERE gid = '$gids' AND  mv.y_sub_value IN ($newStateRange) GROUP BY mv.y_sub_value,mv.x_value,mv.value ORDER BY FIELD(mv.y_sub_value,$newStateRange)";
