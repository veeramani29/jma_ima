	...
	public function createLeftNavigation($nValues,$cat_url_param='',$pIsPremium = false) {
		...
		if(is_array($nValues)) {
					...
					print_r($detalsVals); */
						 ...
						 else
						 {
							...
							switch ($detalsVals['details']['category_type']) {
								...
								case 'M':
								$link_url = url('materials');
								break;
								case 'L':
								$link_url = url($cat_row['details']['category_link']);
								break;
								default:
								$link_url = url('page');
							}
			...
			else
			{
				            $premium_cat_lnk_cls = '';
							if($pIsPremium == false) {
								$isPremium = $granddetalsVals['details']['premium_category'] == 'Y' ? true : false;
							}else{
								$isPremium = $pIsPremium;
							}
							$cat_url = $cat_url_param.$cat_row['details']['category_url']."/";
							...
							if($cat_id == 29){
							  ...
							  $outp.= '<li class="menu__item" role="menuitem"><a class="menu__link" data-submenu="submenu-'.$cat_row['details']['post_category_id'].'" aria-owns="submenu-'.$cat_row['details']['post_category_id'].'" href="#">'.$cat_row['details']['post_category_name'].'</a></li>';
								...
								$outGrants.= '</ul>';
								   	...
								   	$outp.= '<li class="menu__item" role="menuitem"><a class="menu__link"  href="'.$link_url.'">'.$cat_row['details']['post_category_name'].'</a></li>';
							}
			...
			$outp.= '<li class="menu__item" role="menuitem"><a class="menu__link" href="'.$aboutusUrl.'">About IMA</a></li>
				<li class="menu__item" role="menuitem"><a class="menu__link" href="'.$productsUrl.'">Products</a></li>';
				if(isset($_SESSION['user']) && $_SESSION['user']['id'] > 0) {
				$outp.= '<li class="menu__item" role="menuitem">
					<a class="menu__link" href="'.$userMyaccountUrl.'">';
