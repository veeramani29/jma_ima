...
function getcnt($cat_id)
{
	global $db;
	$rs = "SELECT COUNT( * ) AS tot FROM post WHERE DATEDIFF( NOW( ) , `post_datetime` ) <=4 AND post_category_id = '".$cat_id."'";
	$rst = $db->selectQuery($rs);
	return $rst['0']['tot'];
}

function getcnt1($cat_id)
{
	global $db;
	$rs = "SELECT COUNT( * ) AS tot FROM post, post_category WHERE post.post_category_id = post_category.post_category_id AND DATEDIFF( NOW( ) , post.post_datetime ) <=4
	       AND post_category.new_icon_display = 'Y' AND (post_category.post_category_parent_id= '".$cat_id."' OR post_category.post_category_parent_id1= '".$cat_id."')"; 
	$rst = $db->selectQuery($rs);
	return $rst['0']['tot'];
}
...
function makeChart($value){
	...
	{
		$gr_type = 1;
	//	$shwHdeImg = "<img src='images/collapse_icon.png'>";
	$shwHdeImg = '';
		$scrpr_ex = "$('#graph_0').css('width','816px'); graphExt.infoStatus = true;";
		
	} else if($count = preg_match_all('/{graph_narrow ((\d|,|\||-)+)}/', $value, $matches)) {
		$gr_type = 2;
		$shwHdeImg = '';
		$scrpr_ex = "$('#graph_0').css('width','578px'); graphExt.infoStatus = false;";
	}
	if($count > 10) {
		$script = "
...
<script type='text/javascript'>
    ...
    function changeShare(index)
	{
		...
		var link = 'mailto:?subject=India Macro Advisors Graph&body=Personal Message:%0APlease check the chart and its very useful!%0A%0A'+encodeURIComponent(url)+'%0A%0A%0A............................................................................................................%0AIf you are unable to click on the link above, please copy and paste the URL below into a web browser%0A'+encodeURIComponent(url);
		$(this).attr('href', link);
		$('#graph_'+index+' .graph-share').attr('url', url);
		graph.updateUrl(url);
	}

	$('.graph .graph-share').click(function(){
		graph.showShare($(this).attr('url'),$gr_type);
	});
...
chart_files[$index] = ".json_encode(getFilePath($gids)).";
	...
	dashboard[$index].bind(range[$index], chart[$index]);".$createselect;
if($gr_type==1) {
	$gr_right_dv = "<div class='graph-range'>
  <a>1 year</a><a>2 year</a>
  <a>5 year</a><a>10 year</a><a>20 year</a><a>40 year</a>
</div>
</div>
<div class='graph-right' style='background:#ebebeb'>
	<div id='graph_functions_dv'>
		...
		$selection
		<div class='dv_addmore-button'><a class='addmore-button'>Add more</a></div>
		</div>
		<div class='graph-control'>
		<a class='graph-export'>Export</a> &nbsp; 
		<a class='graph-download' href='#' onclick='document.download.submit();'>Download</a> &nbsp; 
		<a class='graph-share'>Share</a> &nbsp; 
		<a class='graph-print'></a>
		</div>
	</div>
	<div id='graph_share_dv' style='display:none;width:207px;height:335px;color:#4f4f4f;'>
		<div id='graph_share_dv_container_div' style='width:207px;'>
				<div id='graph_share_title_dv' style='font-family:arial;font-size:12px;font-weight:bold;color:#010101;margin-top:3px;border-bottom:1px solid #c0c0c0;height:25px'>
					<div style='width:120px;float:left'>E-MAIL THIS CHART</div>
					<a style='display:block;float:right;margin-bottom:10px;' onclick='graph.closeShare($gr_type);'><img src='images/fancybox/fancybox_close.png'></a>
				</div>
				<div id='graph_share_dv_form_div' style='margin-top:20px'>
					<form id='graph_share_form' name='graph_share_form' class='cls_graph_share_form'>
						<div id='graph_share_input_to_email_dv' style='width:207px;height:50px;margin-top:5px'>
							<input type='text' name='graph_share_input_to_email' id='graph_share_input_to_email' class='graph_share_input' placeholder='Your Friend &#39;s Email'>
							<br><span style='font-family:arial;font-size:9px;color:#9a9a9a'>Separate multiple addresses with a comma (,)</span>
						</div>
						<div id='graph_share_input_from_email_dv' style='width:207px;height:36px;margin-top:5px'>
							<input type='text' name='graph_share_input_from_email' id='graph_share_input_from_email' class='graph_share_input' placeholder='Your Email'>
						</div>
						<div id='graph_share_input_link_dv' style='width:207px;height:50px;word-wrap:break-word'>
							
						</div>		
						<div id='graph_share_input_message_dv' style='width:207px;height:50px;margin-top:18px'>
							<input type='hidden'name='graph_share_input_link' id='graph_share_input_link'>
							<textarea name='graph_share_ta_message' id='graph_share_ta_message' class='graph_share_textarea' placeholder='Your Message' style='float:left'></textarea>
						</div>
						<div id='placeholder_error_dv' style='width:207px;margin-top:10px;padding-left:12px;display:none'></div>
						<div style='width:207px;margin-top:10px;height:20px'>
							<input type='submit' value='SEND' class='graph_share_button' style='width:67px;height:20px;margin-left:10px;margin-top:0px;float:right'>
							<input type='button' value='CANCEL' class='graph_share_cancel_button' style='width:80px;height:20px;margin-left:10px;margin-top:0px;float:right' onclick='graph.closeShare($gr_type);'>
						</div>
					</form>
				</div>
				<div id='graph_share_dv_status_div' style='display:none;width:207px;margin-top:60px;'>
					<div style='top: 0; left: 0; bottom: 0; right: 0;margin:auto;'>Sending mail. Please wait.</div>
				</div>
				<div id='graph_share_dv_result_div' style='display:none;width:207px;height:255px;margin-top:10px;'>
					<div style='width:200px;height:200px;position:absolute;top: 0; left: 0; bottom: 0; right: 0;margin:auto;'>
						<div id='graph_share_dv_result_message_div' style='width:200px;text-align:left;font-size:13px'>You've sent this chart</div>
						<div id='graph_share_dv_result_message_div2' style='width:200px;text-align:center;padding-top:10px'><input type='button' value='DONE' class='graph_share_cancel_button' style='width:60px;height:20px;margin-top:0 auto' onclick='graph.closeShare($gr_type);'></div>
					</div>
				</div>
		</div>
		<div style='width:207px;height:20px;font-size:9px;padding-top:15px;font-family:arial'>Note: Details you provide will not be used to send unsolicited e-mail and will not be sold to 3rd parties. <a href='privacy_policy.php' target='_blank' style='color:#2884CC;font-family:arial'>Our privacy policy</a></div>
	</div>
</div></div>";
} elseif($gr_type == 2) {
	$gr_right_dv = "<div class='graph-range1'>
		<a class='graph-download' href='#' onclick='document.download.submit();'>DOWNLOAD</a> &nbsp; 
		<a class='graph-share'>SHARE</a> &nbsp; 
</div>
</div>
<div class='graph-right' style='display:none;background:#ebebeb'>
	<div id='graph_functions_dv'>
		<div class='graph-list'></div>
		<div class='graph-addmore'>
		<span class='addmore'>Add More Series</span>
...
<div id='dv_lnk_graph_show_more' onclick='graphExt.shoHideGraphInfo()'>$shwHdeImg</div>
