...
<script language="javascript">
    $(document).ready(function() {
        $('#add_graph').submit(function() {
			...
			if(Gsource == ''){
                $('#graph_source_error').html('Please enter Map source.');
                ret =  false;
            }
...
function getCol($matrix, $col)
	...
	return $column;
		...
		$id=$_REQUEST['id'];
		$condition="gid = '$id'";
		$getGraphArr = $chartObj->getMapList($condition); //print_r($getGraphArr);
...
######################################
	   ...
	   // edit excel values -- end
		...
		$xfile       = $xlFile;
		$chartObj->updateMapdetails($id,$source,$title,$txt,$is_premium,$xfile);
		// Clear memcached
		try{
		...
		$successMsg = "Map updated successfully.";
	//	echo $successMsg;
		header("Location:editMap.php?id=$id");
		}
?>
...
<div id="page-heading"><h1>Edit Map</h1></div>
