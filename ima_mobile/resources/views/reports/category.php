  ...
  <div class="col-md-10">
	...
	else
	{
		...
		$news_link_url = $this->url(array('controller'=>'reports','action'=>'view','params'=>$category_path.$resn[$i]['post_url']));
			?>
		  <div class="col-md-12 <?php if ($i == 0) echo "first-section";?> <?php if ($i == $count - 1) echo "last-section";?>"><?php if ($resn_date > $today) echo '<p class="latest">Latest</p>'; ?>
  <?php if(!empty($resn[$i]['post_released'])) {?><p class="released <?php if ($resn_date > $today) echo "first"; ?>"><?php echo stripslashes($resn[$i]['post_released']);?></p><?php }?>
			<p class="title"><a  class="title-link" href="<?php echo $news_link_url;?>"><h3 class="title"><?php echo stripslashes($resn[$i]['post_title']);?></h3></a></p>
