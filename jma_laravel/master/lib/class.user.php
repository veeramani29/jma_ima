...
class user {
	...
	*/
	...
	function getSortUserList($userType=null,$userStatus=null){
		$this->db->open();
		$where = "";
		$table = "";
		if($userType !=null && $userStatus ==null)
		{
			if($userType=="Linked")
			{
				$table = ", `linkedin_user_account` linkedin_user_account ";
				$where = "AND usr.id = linkedin_user_account.user_id AND usr.linkedin_enabled = 'Y'";
			}
			elseif($userType=="Login")
			{
				$where = "AND usr.linkedin_enabled = 'N'";
			}
		}
		elseif($userType == null && $userStatus !=null)
		{
			$where = "AND usr.user_status_id = '".$userStatus."'";
		}
		elseif($userType !=null && $userStatus !=null)
		{
			if($userType=="Linked")
			{
				$table = ", `linkedin_user_account` linkedin_user_account ";
				$where = "AND usr.id = linkedin_user_account.user_id AND usr.linkedin_enabled = 'Y' AND usr.user_status_id = '".$userStatus."'";
			}
			elseif($userType=="Login")
			{
				$where = "AND usr.linkedin_enabled = 'N' AND usr.user_status_id = '".$userStatus."'";
			}
		}
		
	   
		
		 $query_pag_data   = "SELECT usr.*, usertype.type_name 
		AS user_type_name, usertype.type_description AS user_type_desc, userstatus.status_name 
		AS user_status_name, userstatus.status_description AS user_status_desc 
		FROM `user_accounts` usr, `user_types` usertype, `user_statuses` userstatus ".$table."
		WHERE usertype.`id` = usr.`user_type_id`  ".$where."
		AND userstatus.`id` = usr.`user_status_id` ORDER BY usr.`id` DESC";
		$postList = $this->db->fetchArray($this->db->query($query_pag_data));
		$this->db->close();
		return $postList;
	}
	
	function getIndiviualLinkedInUser($id){
		$this->db->open();
		$query_pag_data   = "SELECT * FROM `linkedin_user_account` WHERE `user_id` = '".$id."'";
		$postList = $this->db->fetchArray($this->db->query($query_pag_data));
		$this->db->close();
		return $postList;
	}
        ...
        function updateUserProfile($profile,$uid){
        	...
        	foreach ($profile as $field => $postVal){
        		if($field == 'company_id' && $postVal == ''){
        			...
        			$sql_update.="`$field` = '".mysqli_real_escape_string($this->db->connection,$postVal)."', ";
        		}
        	}
