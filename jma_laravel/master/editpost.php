...
$insertArray['recent_data'] = $myPosts[0]['recent_data'];
			...
			$errorMsg ='Please enter all mandatory fields<br/>';
			   ...
			   //$postObj->updatePost($updateId,$category,$writer,$postTitle,$shortDesc,$postHead,$subHead,$postReleased, $post,$postType);
		        try {
					...
					$variables = array("Recent data trend :", "Recent data trend:","Brief","Recent data trend","http://","https://"," :");
          $replacements   = array("","","","","","","");

					  $resent_data = str_ireplace($variables, $replacements, preg_replace('/\s\s+/', '',$matches[0]));
									...
									else
									{
								     ...
								     $mailTemplate = $postObj->selectMailTemplate('14');
										...
										$result = $mgClient->sendMessage($domain, array(
												'from'    => 'Japan Macro Advisors <takuji.okubo@japanmacroadvisors.com>',
												'to'      => $listAddress,
												'subject' => $mailSubject,
												'html'    => $mailBody,
    ...
    array_walk_recursive($array, function(&$item, $key){
     ...
     return $array;
								  ...
								  $usersListByLimit_=utf8_converter($usersListByLimit);
								        $userListformembers = json_encode($usersListByLimit_);
		...
		<!-- start id-form -->
				...
				<div class="error_sent_notification"><?php echo $errorMsg;?></div>
	             <?php } ?>
                    ...
                    <td >
                            <input type="radio" value="Y" name="recent_data" id="recent_data"   <?php if($insertArray['recent_data'] == 'Y') { echo "checked"; } ?> /> Yes
							<input type="radio" value="N" name="recent_data" id="recent_data"   <?php if($insertArray['recent_data'] == 'N') { echo "checked"; } ?> />  No
