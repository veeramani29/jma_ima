 @extends('templates.default')
@section('content')
 <div class="col-md-7 col-xs-12">
  <div class="row">
    <div class="col-xs-12">
      <div class="main-title">
        <h1> Notification based on the Specified Commercial Transactions Act</h1>
        <div class="mttl-line"></div>
      </div>
      <table class="table table-bordered commtbl" >
        <tbody>
          <tr>
            <td>Legal Name</td>
            <td>Japan Macro Advisors Inc.</td>
          </tr>
          <tr>
            <td>Address</td>
            <td>
              Roppongi Hills North Tower Level 17 Roppongi 6-2-31, Minato, Tokyo, 106-0032, Japan
            </td>
          </tr>
          <tr>
            <td>Phone number</td>
            <td>03-5786-3275</td>
          </tr>
          <tr>
            <td>Email address</td>
            <td><a href="mailto:support@japanmacroadvisors.com">support@japanmacroadvisors.com</a></td>
          </tr>
          <tr>
            <td>Business hours</td>
            <td>10:00 ～ 17:00
              <br>Closed on Saturday, Sunday, National holidays
            </td>
          </tr>
          <tr>
            <td>Director of operations</td>
            <td>Takuji Okubo</td>
          </tr>
          <tr>
            <td>Additional fees</td>
            <td>Not applicable</td>
          </tr>
          <tr>
            <td>Exchanges & Returns</td>
            <td>Due to the nature of digital subscription service, no amendments, cancellations, or refunds will be possible after purchase of the service. Your understanding is appreciated.</td>
          </tr>
          <tr>
            <td>Delivery times</td>
            <td>Immediate after the completion of the payment</td>
          </tr>
          <tr>
            <td>Accepted payment methods</td>
            <td>See our <a href="<?php echo url('products');?>">product</a> page</td>
          </tr>
          <tr>
            <td>Payment period</td>
            <td>Credit card payments are processed immediately. </td>
          </tr>
          <tr>
            <td>Payment</td>
            <td>In advance</td>
          </tr>
          <tr>
            <td>Value</td>
            <td>¥3,500</td>
          </tr>
        </tbody>
      </table>
    </div>
  </div>
</div>
 @include('templates.rightside') 
@stop