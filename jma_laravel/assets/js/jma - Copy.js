...
function YieldChart(p_chartIndex, chartDetails) {
	...
	{
		 $('body').on('click','.exhibit-tab-footer_'+chartCommon.Conf.chartIndex+' li', function(e) {
				e.preventDefault();
				e.stopPropagation();
				...
				$exhibit.find('.exhibit-tab-footer_'+chartCommon.Conf.chartIndex+' li').removeClass('selected');
				...
				if(view == "data")
				{
					...
					var dataTable_container = '#Table_Dv_placeholder_'+chartCommon.Conf.chartIndex;
					var small_div='';
	 		        var dynamictd=82;
					if(chartCommon.Conf.chartData !=null) {
						
						var myObject = chartCommon.Conf.chartData;
						var count = Object.keys(myObject).length;
                        
						
						var out = '<table cellspacing="0" cellpadding="0" class="mychart_table fixed_headers table table-striped"><thead><tr><th>Maturity</th>';
						var out_data = '';
						var data_formated = [];
						var column_width = dynamictd/count;
						
						$.each(chartCommon.Conf.chartData,function(order,series){
							
							out+="<th width='"+column_width+"%'>"+chartCommon.Conf.chart_labels_available[order]+"</th>";
							$.each(series,function(i_order,dataset){
								...
								else
								{
									...
									if(chartCommon.Conf.chart_data_type == 'monthly'){
										var dte = Highcharts.dateFormat('%b',datasets)+" "+Highcharts.dateFormat('%Y',datasets);
									}else if(chartCommon.Conf.chart_data_type == 'quaterly'){
					...
					$('.h_graph_content_area_'+chartCommon.Conf.chartIndex).hide();
					//$('#footer_chart_dataDoenload_'+chartCommon.Conf.chartIndex).show();
				    ...
				    $(this).addClass('selected'); 
					$('.h_graph_content_area_'+chartCommon.Conf.chartIndex).show();
					$('#Table_Dv_placeholder_'+chartCommon.Conf.chartIndex).empty();
					//$('#footer_chart_dataDoenload_'+chartCommon.Conf.chartIndex).hide();
				}		
		});
		
	    
		
		if($('.h_graph_content_area_'+chartCommon.Conf.chartIndex).css('display') == 'none')
		{ 
	...
	this.createChartDataSeries = function() {
		...
		if(chartCommon.Conf.chartColorSatus == 'true' || chartCommon.Conf.color_status == 'true')
		{
						...
						$('.basicsss1:eq('+index+')').spectrum({color: '#ECC',showInput: true,className: 'full-spectrum',
								...
								change: function() {
									$('.basicsss1:eq('+index+')').each(function(idx) {
										...
										var newSettingColor = chartCommon.chartConfigs.colors[indexChart1] = codeColor;
	...
	this.createMultiYaxisConfigurations = function(chart_data_series){
		...
		$.each(chart_data_series,function(ky,chData){
			...
			{
				var comCol = ky - 1;
				if(chartCommon.Conf.chartColor[comCol])
				{
					var seriesIndx = chartCommon.Conf.chartColor.length;
					var newIndex = ky - seriesIndx;
					colorText =chartCommon.chartConfigs.colors[newIndex];
				}
				else if(chartCommon.Conf.mychartchartColor !== undefined)
				{
					if(chartCommon.Conf.mychartchartColor[comCol])
					{
						var seriesIndx = chartCommon.Conf.mychartchartColor.length;
						var newIndex = ky - seriesIndx;
						colorText = chartCommon.chartConfigs.colors[newIndex];
					}
				}
				...
				{
					colorText = chartCommon.chartConfigs.colors[ky];
				}
    ...
    //     shadow: false,
				positioner: function () {
            return { x: 37, y: 285 };
        },followTouchMove: false,shared: true,style: tooltipstyle, };
...
this.drawJmaChart = function(){
	...
	this.enableSwitchingChartToTable();
...
function LineChart(p_chartIndex, chartDetails) {
						...
						$('.basicsss1:eq('+index+')').spectrum({color: '#ECC',showInput: true,className: 'full-spectrum',
								...
								change: function() {
									$('.basicsss1:eq('+index+')').each(function(idx) {
										...
										var newSettingColor = chartCommon.chartConfigs.colors[indexChart1] = codeColor;
										//console.log(chartCommon.chartConfigs.colors); return false;
									    changeColorofCharts_(indexChart,codeColor,indexChart1);
									});
    ...
    //     shadow: false,
				positioner: function () {
            return { x: 37, y: 285 };
        },followTouchMove: false,style: {}, /*formatter: function () {
				 var s = '';
				...
				chart : {
					...
					exporting : {
						...
						chartOptions:{
							...
							events : {
								load : function(){
									this.renderer.image(window.location.protocol+'//content.japanmacroadvisors.com/images/jma-logo-export.png', 385, 300, 195,16).add();
									this.renderer.text("Source : "+chartCommon.Conf.sources, 10, 310, 159, 33).css({size:'3px'}).add();
								}
							}
						},
...
function JMAChart(){
	...
	$.ajax({
		...
		dataType : 'json',
		type : 'POST',
		data : {'type' : data_type, 'chartcodes' : chartcodes, 'chartColor' : chartcolor},
		beforeSend: function() { JMA.showLoading(); },
		success : function(data){
	 ...
	 */
	 this.updateThisEditedChart = function(){
	 	...
	 	try {
			...
			//console.log(colors); return false;
...
function IsCharacter(evt) 
{
	...
	function All_share(idx,shareNetwork)
	{
		...
		var chart_svg = chart.getSVG({
			...
			width:((JMA.JMAChart.Charts[idx].chart_object.legend.legendWidth<400)?JMA.JMAChart.Charts[idx].chart_object.legend.legendWidth:JMA.JMAChart.Charts[idx].chart_object.legend.maxItemWidth)+15,
