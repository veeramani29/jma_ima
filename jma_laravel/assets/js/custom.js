...
$(function() {
  ...
  $("#signup_frm").validate({
    submitHandler: function(form) {
      ...
      LastName();
      ValidateCaptcha();
      if (JMA.flag == true && JMA.flags == true && JMA.captcha == true) {
        form.submit();
        return true;
      } 
      if (JMA.flags == false) {
...
// search header
$(".sea-jmaind").click(function(){
  $(".sea-indinp").slideToggle(500);
});
$( function() {
  var availableTags = [
  "GDP",
...
$("#reg_first_name").on('paste keypress keydown blur keyup', function() {
  if(this.value!=''){
    ...
    if((/^[a-zA-Z]*$/.test(str) == false)) {
      var str1=str.replace(/[^a-zA-Z]+/gi,'');
      $(this).val(str1);
      return false;
    }
...
$(document).ready(function(){
  // mobile menu
  $('.mob_maitit').on('click', function(e){
    e.preventDefault();
    $(this).next('.mob_submen').slideToggle();
  });
});
