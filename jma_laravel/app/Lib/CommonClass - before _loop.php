...
$text_msg="<p>If you are already a JMA subscriber please <a class='subscriber_login' ".$popup_link_url." >login</a></p>";
          ...
          if ($count = preg_match_all("/{(graph[_a-z]*) ((\d|,|\||-)+)({([a-zA-Z:,\"\'&quot;]*)})*}/", $value, $matches)) {
               ...
               if ($count > 0) {
                    ...
                    for ($cnt_graphs = 0; $cnt_graphs < $count; $cnt_graphs++) {
                         ...
                         // Get Chart data
                         $chart_prefetch_data  = $this->getchartData($GIDS, $data_period_type, $availableChartLabels, $all_the_gids,$cnt_graphs,$graph_default_configs['chartLayout']);
     ...
     public function getchartData($gids, $data_type, $availableChartLabels, $all_the_gids = NULL,$cnt_graphs = NULL, $chartLayout = NULL)
     {
          ...
          if ($indicator_values_m != false && count($indicator_values_m) > 0) {
               ...
               foreach ($gids as $gid) {
                    ...
                    if (count($cvalues) >= 1) {
                         ...
                         $qry    = str_replace('%', '\%', "select $x_value `year`, `month`, `date`, `value` from `graph_values` where `gid`='" . $cids[0] . "' and `y_value` like '" . $cvalues[0] . "' and `y_sub_value` like '" . $cvalues[1] . "' and not `value` like '' order by `year` $order_by");
                         $result = $graphValues->getThisqueryResult($qry);
                         
                         if (!$result) {
                              $qry_sub = str_replace('%', '\%', "select `year`, `month`, `date`, `value` from `graph_values` where `gid`='" . $cids[0] . "' and `y_value` like '" . $cvalues[0] . "' and `y_sub_value` like '" . $cvalues[1] . "' order by `year`, `month`, `date`");
