...
class StripePayment {
		...
		try{	
				...
				try {
					...
					if (!isset($stripeToken)) 
						...
						$plan=(isset($_POST['plan'])?$_POST['plan']:"jma_premium_trial");
	...
	public function createCharge($stripeToken, $email){
				...
				try {
					...
					if (!isset($stripeToken)) 
						...
						$charge = \Stripe\Charge::create(array(
						  "amount" => 10000,
						  "currency" => "usd",
						  "description" => "Custom discount charge",
						  "source" => $token
						  //"metadata" => $metadata,
						));
	...
	Public function upgradeStripeSubscription($cus_id){
		try{
			\Stripe\Stripe::setApiKey($this->api_key);
			$customer = \Stripe\Customer::retrieve($cus_id);
			$upgrade = $customer->subscriptions->create(array("plan" => 'premium_without_trial'));
			return $upgrade;
		}catch (Exception $ex) {
			//throw new Exception($ex->getMessage(), $ex->getCode());
			return $ex->getMessage();
	...
	public function getCustomerFromEvent_($event_customer){
		try {
			\Stripe\Stripe::setApiKey($this->api_key);
			$customer = \Stripe\Customer::retrieve($event_customer);
			return $customer;
	...
	public function getCustomerPlan_($event_customer){
		try {
			\Stripe\Stripe::setApiKey($this->api_key);
			$customer = \Stripe\Customer::retrieve($event_customer);
			return $customer;
