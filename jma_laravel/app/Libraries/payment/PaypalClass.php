...
class PaypalClass {
	...
	public function __construct() {
		...
		$this->api_version = $api_configurations['api_version'];
			...
			define('DESC', "Subscription fee of ".Config::read('subscription.currency').Config::read('subscription.amount')." will be charged once the 30 days free trial period is over.");
	...
	public function createExpressCheckout($order_id, $transaction_id, $iAparams = array()){
		try{
			...
			$params['L_PAYMENTREQUEST_0_NAME0'] = 'JMA Standard Trial Subscription';
			$params['L_PAYMENTREQUEST_0_DESC0'] = 'First month trial period subscription fee.';
			$params['L_PAYMENTREQUEST_0_AMT0'] = 0.00;
	...
	public function confirmPayment($order_id, $transaction_id, $payment_paypal_token,$payer_id,$final_amount) {
		try{
			$params = array();
	...
	public function confirmPaymentAndInitiateSubscription($uid, $order_id, $transaction_id, $payment_paypal_token, $payer_id, $final_amount, $user_email, $user_name, $subscription_start_date) {
		...
		if($check === true) {
			...
			$params['SIGNATURE'] = $this->api_signature;
			$params['TOKEN'] = $payment_paypal_token;
			$params['PROFILESTARTDATE']=gmdate("Y-m-d\TH:i:s\Z",strtotime('+1 day'));
			...
			$params['L_PAYMENTREQUEST_0_NAME0'] = 'JMA Standard Monthly Subscription';
