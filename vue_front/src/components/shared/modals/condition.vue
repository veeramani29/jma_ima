<template>
  <v-content>
    <v-dialog v-model="modalData" scrollable persistent max-width="790">
      <v-card>
        ...
        <v-card-text>
          ...
          <p>
            ...
            <u>Each individual user must have their own individual subscription.</u>
          </p>
          <div class="sub-title">
          ...
          <p>
            Free subscription term/period is unlimited. Premium subscription will end after the expiration date until you cancel the subscription. Once you cancel your subscription you will be notified via email and your subscription type will turn back to Free subscription. If you encounter any problem, please feel free to contact us at
            <a
              href="mailto:support@indiamacroadvisors.com"
            >support@indiamacroadvisors.com</a> or you can submit your query at our
            <a href="#">Help Desk</a>.
          </p>
          <div class="sub-title">
            <h5>No Refund</h5>
        ...
        <v-card-actions>
          ...
          <v-btn color="green darken-1" text @click="toggleDisagree()">Disagree</v-btn>

          <v-btn color="green darken-1" text @click="toggleAgree()">Agree</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-content>
...
export default {
  computed: {
    modalData() {
      return this.$store.state.termConModal;
    },
    agreeData() {
  ...
  methods: {
    ...
    toggleDisagree() {
      return (
        (this.$store.state.termConModal = !this.$store.state.termConModal),
        (this.$store.state.agreeTerm = false)
      );
    },
    toggleAgree() {
      return (
        (this.$store.state.agreeTerm = true),
        (this.$store.state.termConModal = !this.$store.state.termConModal)
      );
    }
  }
