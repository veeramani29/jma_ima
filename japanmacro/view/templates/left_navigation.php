<div class="col-xs-12 col-md-2 content_leftside">
  <div class="smacol_leftmenu" role="tablist" aria-multiselectable="true">
    <ul class="menu top notranslate list-unstyled" id="accordion" > 
		...
		<?php $chartBook = $this->resultSet['result']['category']['chartBookList']; 
		  if(!empty($chartBook)) { ?>
				...
				<?php if(count($chartBook)>3){ ?>
					<a href="mycharts/list_chartbook" class="cbt_more"><img src="<?php echo $this->images;?>more-list.png"></a>
				<?php } ?>
				</div>
				</li>  
       ...
       <?php if(isset($this->resultSet['result']['category']['menu'])) {
        echo $this->resultSet['result']['category']['menu']; } ?>
	  
	
	
	  <?php $showtBookList = $this->resultSet['result']['category']['showtBookList']; 
	  ...
	  if(!empty($showtBookList)) { ?>
	   ...
	   <ul class="list-group">
		...
		{
			$out.= '<li class="submenu_leftside list-group-item"><i class="fa fa-book" aria-hidden="true"></i><a data-id="'.$values['folder_id'].'"  href="mycharts/listChartBook/#'.$values['folder_id'].'" class=" content_leftside_parent">'.$values['folder_name'].'</a></li>';
		}
		echo $out;
		?>
        </ul>
