 ...
 var yAxis = {
	...
	if(isBig.matches)
	{
		...
		var wordwapF = function() {
			...
			credits : {
				enabled : false,
				href : 'http://japanmacroadviaord.com',
	...
	this.exportChart = function(idx,pType,pSize){
		...
		if(type=='ppt'){
			...
			JMA.JMAChart.Charts[idx].chart_object.exportChart({
				...
				sourceHeight : 340,
				filename : 'jma_chart_'+type+'_'+width_val+'_'+idx,
		//	url : 'http://japanmacroadvisors.com/chart/exportChart'
	});
		}
	 			...
	 			$.ajax({
	 				...
	 				success : function(response){
	 					if(response.status!=1){
							...
							if(document.getElementsByClassName('newclassforcb').length>2)
							{
	 			...
	 			$.ajax({
	 				...
	 				success : function(response){
	 					if(response.status!=1){
							...
							var content = '<div class="modal-dialog modal-sm" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title" id="myModalLabel">Delete Chart Book</h4> </div><div class="modal-body">Click on the '+satus+' button to '+satus+' the chartbook.</div><div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button><button type="button" data-id="'+satus+'-'+folderId+'" class="btn btn-primary status-chartbook-name">'+satus+'</button> </div></div> </div>';
	 	...
	 	if(Array.isArray(response) == true && response.length > 0){
	 		...
	 		$.each(response,function(order,folderContent){
	 			if(order==0){
	 				...
	 				listview_content+='<div class="main-title list_minttl"><h1 class="">'+folderContent.folder_name+'<a href="#" class="print-mycharts"> <i class="fa fa-print"></i> <span>Print</span> </a></h4><div class="mttl-line"></div></div>';
	 			}
	 			if(order%4 == 0){
	 				pageCount++;
...
$(grids).append('<div class="col-sm-8 padl0 page-title page-title-no-line"><h4 class=""></h4></div>');
	...
	var smallviewfolder_title='<h1>'+self.myFolder.currentFolder.name+'<a href="#" class="print-mycharts"> <i class="fa fa-print"></i><span>Print</span></a></h1><div class="mttl-line"></div>';
	 		...
	 		switch (this.chartType) {
	 			...
	 			if(JMA.myChart.myFolder.currentView=='smallView'){
	 				...
	 				if (mq.matches) {
	 					...
	 					this_chart_object_resize.setSize(425, 200);
	 ...
	 */
	 this.exportChart = function(idx,pType,pSize){
	 	...
	 	self.chart_object.exportChart({
			...
			//url : 'http://localhost/jma/japanmacroadvisors.com/chart/exportChart'
		//	url : 'http://testing.japanmacroadvisors.com/chart/exportChart'
...
$(document).on('click','li.nav_edittabs',function(){
	...
	function hideCollapseBox(type,defltVal)
	{
		...
		if(type==2)
		{
			document.getElementById("alert_value").value = defltVal;
		}
