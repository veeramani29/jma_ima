...
@endif
  <form class="form-horizontal cmxform signup_frm" name="signup_frm" id="signup_frm"  method="post" autocomplete="off">
     ...
     <!--  <li class="ssp_freuse col-xs-12 col-sm-4">
        <div class="form-group">
          <div class="free_user signup_request_select <?php echo (!isset($product)) ? 'activepro': '';?> <?php echo (isset($product) && $product =='free') ? 'activepro': '';?>" >
            ...
            <span class="fa fa-check pull-right" <?php echo (!isset($product)) ? 'style="display:block"': '';?> <?php echo (isset($product) && $product =='free')? 'style="display:block"': 'style="display:none;"';?>></span>
          </div>
        </div>
      </li> -->
      <li class="ssp_preuse col-xs-12 col-sm-4">
        <?php //echo (isset($product) && $product =='premium') ? 'activepro': '';?>
        <div class="signup_request_select activepro" >
    ...
    <div class="form-group text-center">
      <a href="<?php echo url('user/linkedinProcess',isset($product)? $product : 'premium');?>" class="reg_linkedin btn btn-lisu">
        <i class="fa fa-linkedin"></i> Sign up
      </a>
      <a class="reg_fb btn btn-fbsu" onclick="window.open(this.href, '_blank','width=600,height=600,scrollbars=yes,status=yes,resizable=yes,screenx='+((parseInt(screen.width) - 600)/2)+',screeny='+((parseInt(screen.height) - 600)/2)+'');return false;" href="<?php echo url('auth/login/facebook',isset($product)? $product : 'premium');?>"> 
        <i class="fa fa-facebook"></i> Sign up
      </a> 
      <!-- <div class="col-xs-6 text-center">
        <a class="text-center reg_linkedin"   title="Linkedin Signup" href="<?php echo url('user/linkedinProcess',isset($product)? $product : 'free');?>">
          <img style="height: 42px;" alt="Linkedin Signup" src="<?php echo images_path('linkedin_signup.png');?>" />
        </a>
      </div>
      <div class="col-xs-6 text-center">
        <a href="<?php echo url('auth/login/facebook',isset($product)? $product : 'free');?>"
         class="text-center reg_fb" 
         onclick="window.open(this.href, '_blank','width=600,height=600,scrollbars=yes,status=yes,resizable=yes,screenx='+((parseInt(screen.width) - 600)/2)+',screeny='+((parseInt(screen.height) - 600)/2)+'');return false;" title="Facebook Signup" > 
    ...
    <div class="col-xs-12 suf_tercon">
      ...
      <div style="display: inline-block;" class="text-center">
        ...
        <label class="control control--checkbox">
              Agree with the <a data-toggle="modal" data-target=".tercon-modal"> terms and conditions </a><br>
			 
              <input type="checkbox" value="y" id="agreeButton" <?php echo (isset($agreeButton) && $agreeButton=='y')?'checked':'';?> name="agreeButton" class="required">
  ...
  <div class="form-group">
      <div class="col-xs-12">
        <div class="g-recaptcha" name="g-recaptcha" id="g-recaptcha" data-sitekey="6Ldi32wUAAAAAADV4QEghtctMIptO3e7WS8i94N6"></div>
