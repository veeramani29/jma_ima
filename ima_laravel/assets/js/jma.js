...
define(["mobiledetect"],
	...
	this.Export_url=(window.location.protocol=='http:')?window.location.protocol+'//export.highcharts.com':window.location.protocol+'//export.highcharts.com';
	this.__construct = function(appURL,appController,appAction,appParams,objectParams){
		...
		$(document).ready(function() {
			...
			if((self.controller=='page' && (self.action=='category' || self.action=='ideapitchcompetition')) || (self.controller=='reports' && self.action=='view')){
...
if(urlParams.has('graph_index')){
if(urlParams.get('graph_index')!=''){
	// var elmnt=document.getElementById('Jma_chart_container_'+parseInt(urlParams.get('graph_index')));
	// var elmnt = (Jma_chart_container_'+parseInt(urlParams.get('graph_index')));
	 // elmnt.scrollIntoView(true).offset().top - 300;
	 // $(document).scrollTop(elmnt).offset().top - 100; 
	 // $( window ).on( "load", readyFn )
$( window ).on('load', function (e) {
    $('html, body').animate({
        scrollTop: $('#Chart_Dv_placeholder_'+parseInt(urlParams.get('graph_index'))).offset().top -50
    }, 2000);
});
}
}
...
return out_data;
    ...
    if (md.mobile() || md.tablet()) {
      ...
      // $('[id^="Chart_Dv_placeholder_"] .addmor-menu').dlmenu();
    } else {
...
function mapsChart(p_chartIndex, chartDetails) 
{
				...
				this.createHighChart = function() 
				{
								   ...
								   { // on complete
										mapChart.renderer.image(window.location.protocol+'//www.indiamacroadvisors.com/assets/images/logo.png', 390, 495, 180, 12).add(); 
										 ...
										 var	countryChart = new Highcharts.chart('state_chart_'+mapCommon.Conf.chartIndex, {
												...
												chartOptions:{
													chart : {
														...
														events : {
															load : function(){
																this.renderer.image(window.location.protocol+'//www.indiamacroadvisors.com/assets/images/logo.png', 10, 385, 150,10).add();
										  ...
										  , function (chart) {	
											//chart.renderer.image(window.location.protocol+'//www.indiamacroadvisors.com/assets/images/logo.png', 80, 288, 195, 12).add();
		    ...
		    //   $('[id^="Chart_Dv_placeholder_"] .addmor-menu').dlmenu();
		    // } else {
		    //   $('a.btn-admor').on('click', function(event) {
		    //     event.preventDefault();
		    //     event.stopPropagation();
		    //     $(this).parent().siblings().removeClass('dropdown-clicked');
		    //     $(this).parent().children('.dropdown-menu').children('.dropdown-clicked').removeClass('dropdown-clicked');
	...
	this.enableSwitchingChartToTable = function()
	{
		...
		$('body').on('click','.exhibit-tab-footer_'+chartDetails.Conf.chartIndex+' li', function(e) {
				...
				{ 
					...
					chartDetails.Conf.view_option = 'table';
					$('ul.list_graphhead').find('li:first,li:nth-child(2)').hide();
					  $('[data-toggle="tooltip"]').tooltip();
					$('ul.list_graphhead').find('li:nth-child(3) div i').attr('data-original-title','Export Table');
					...
					$(".h_graph_graph_area").focus();
					// alert("test");
					 //  $(".tick").find("text").find("tspan").each(function(){
					 ...
					 //  	string.replace(/(^\s*)|(\s*$)/gi,"");
						...
						// string.replace(/\n /,"\n");
					 //    var string_count=string.split(' ').length;
					 //    if(string_count > 3){
					 //    	var first_three_string=string.split(' ').slice(0,3).join(' ');
					 //    	var next_three_string=string.split(' ').slice(3,6).join(' ');
					 //    	$(this).text(first_three_string);
					 //    	$(this).after('<tspan x="0" y="10" dy="1.7em">'+next_three_string+'</tspan>');
					 //    }
					 //  });
				...
				$('ul.list_graphhead').find('li:first,li:nth-child(2)').show();
				  $('[data-toggle="tooltip"]').tooltip();
					$('ul.list_graphhead').find('li:nth-child(3) div i').attr('data-original-title','Export  Chart');
				    $(this).addClass('selected');
...
//console.log(jsonObj);//return false;
						var tableHeadCount = tableHead.length;
							var tableTspanCount = chartDetails.Conf.current_chart_codes.length+1;
...
var style='g.svg-header g.tick text tspan {font-size:7px;word-break: break-word;width:200px; word-spacing: 1px;}g.chartGroup g.row text tspan {font-size:10px;}';
				...
				headerSvg.append("rect").attr("rx", 0).attr("ry", 0).attr("height", "30").attr("width", "100%").attr("fill-opacity", 1).attr("style","fill:#ddd;");
	 				...
	 				var thvalues = 'Date';
								 ...
								 var md = new MobileDetect(window.navigator.userAgent);
            					if (md.mobile() || md.tablet()) {
            						var tableHeadMobile = 75;
            					} else {
            						var tableHeadMobile = 150;
            					}
									...
									.text(thvalues).call(svg_word_wrap, tableHeadMobile);
								...
								headerSvg.insert("image")
									.attr("x", 500).attr("y", 370).attr("width", 100).attr("height",20).attr("xlink:href", 'https://www.indiamacroadvisors.com/assets/images/favicon.png');
...
var tfm = fixed.transform.baseVal.getItem(0);
 tfm.setTranslate(0, event.target.scrollTop);
	...
	this.mobilelegent = function(chart_data_series)
	{
 ...
 var yAxis = {
	...
	if(isBig.matches)
	{
		...
		var wordwapF = function() {
			...
			p_chrtObj.renderer.image(window.location.protocol+'//www.indiamacroadvisors.com/assets/images/logo.png', (chartCommon.Conf.chartLayout=='narrow' && JMA.controller!='home')?700:365, 380, 218,17).add();
            ...
            connectNulls: true ,
                dataLabels: {
					...
					p_chrtObj.xAxis[0].setExtremes(
						chartCommon.Conf.navigator_date_from,
						chartCommon.Conf.navigator_date_to);
					//console.log(p_chrtObj.userOptions.series.length);
...
if(JMA.userDetails.hasOwnProperty('id') && JMA.userDetails.id>0) {
	if(JMA.userDetails.user_permissions.graph.datadownload.allowdatadownload == 'Y') {
       ...
       JMA.User.showUpgradeBox('download',idx);
    }
}else{
	JMA.User.showLoginBox('download',idx);
}
...
chart_svg = '<svg class="scroll-svg" version="1.1"  xmlns="http://www.w3.org/2000/svg" width="600" style="font-family:&amp;quot;Lucida Grande&amp;quot;, &amp;quot;Lucida Sans Unicode&amp;quot;, Arial, Helvetica, sans-serif;font-size:12px;"  height="410">'+find_image+'</svg>';
...
this.get_chart_fields_labels = function(p_chart_idx,p_graph_code){
	...
	this.addThisGraphCode = function(p_chart_idx,p_graph_code){
		...
		var thing = this;
		setTimeout(function() {
		//console.log(JMA.JMAChart.Charts[p_chart_idx].Conf.charts_fields_available[series_main_code]);
...
return weekenddate; 
   ...
   this.All_share = function(idx,shareNetwork){
	...
	this.reverseYAxis = function(p_chart_idx){
			...
			var reversed = JMA.JMAChart.Charts[p_chart_idx].chart_object.yAxis[0].reversed;

			for(var yX=0; yX<JMA.JMAChart.Charts[p_chart_idx].chart_object.yAxis.length; yX++) {
						JMA.JMAChart.Charts[p_chart_idx].chart_object.yAxis[yX].update({ reversed: !reversed });
				    }
			reversed = !reversed;
	};
		...
		charts_available : JMA.JMAChart.Charts[chartIndex].Conf.charts_available
				...
				$.ajax({
					...
					success : function(response){
						console.log(JSON.stringify(response, null, 4));
						if(response.status!=1){
							// alert(JMA.userDetails.user_type);	
							if(response.status == 1001){
								if(JMA.userDetails.user_type == 'individual' || JMA.userDetails.user_type == 'corporate'){
									self.showFolderCreationRestricted();
								}else{
							...
							if (md.mobile() || md.tablet()) {
			            		$("#save-success-alert").fadeTo(2000, 500).slideUp(500, function() {
							      $("#save-success-alert").slideUp(500);
							    });
							}
						}
						JMA.hideLoading();
	 ...
	 this.editThisFolderContent = function(pType,order,pUuid){
		...
		var chartDetailsObj = {
			...
			chart_config : {
				...
				reverseYAxis : self.myFolder.currentFolder.charts[chart_index].reverseYAxis,
	 		...
	 		if(this.data.chartDataSeries!=null) {
	 			...
	 			$.each(self.data.chartDataSeries,function(order,series){
	 				out+="<th width='"+column_width+"%'>"+series.name+"</th>";
	 					...
	 					if(self.current_chart_codes.length>1){ //console.log(series);
							var StartYr=series[0][0];var findYr = new Date(StartYr);StartYr=findYr.getFullYear();
							var EndYr=series[series.length-1][0]; var findEYr = new Date(EndYr);EndYr=findEYr.getFullYear();
									var quat='%b ';
           ...
           fallbackToExportServer: false,
					chartOptions:{
						chart : {
							...
							events : {
								load : function(){
									this.renderer.image(window.location.protocol+'//www.indiamacroadvisors.com/assets/images/logo.png', 385, 315, 195,12).add();
			...
			if(JMA.myChart.myFolder.currentView=='largeView'){
			p_chrtObj.renderer.image(window.location.protocol+'//www.indiamacroadvisors.com/assets/images/ima-logo-80_80_small.png', 450, 380, 60,16).add();
			p_chrtObj.renderer.text("Source : "+self.data.sources, 10, 390, 159, 33).add();
		}
		p_chrtObj.yAxis[0].update({reversed: self.reverseYAxis });
			/* p_chrtObj.xAxis[0].setExtremes(
...
//console.log(diffDays);
			...
			if(JMA.myChart.myFolder.currentView=='largeView'){
			p_chrtObj.renderer.image(window.location.protocol+'//indiamacroadvisors.com/images/jma-logo-80_80_small.png', 450, 380, 60,16).add();
			p_chrtObj.renderer.text("Source : "+self.data.sources, 10, 390, 159, 33).add();
		}
			p_chrtObj.xAxis[0].setExtremes(
				self.navigator_date_from-1,
...
//console.log(diffDays);
			...
			for(var yX=0; yX<p_chrtObj.userOptions.series.length; yX++) {
					p_chrtObj.yAxis[yX].update({reversed: self.reverseYAxis });
				    }
		});
	    // Drawn Highchart		
		// **************************************************			
...
/*
$(document).on('change','.submenu_addmore select',function(e){
$(this).parents(".graph-addmore").find(".submenu_addmore select[id!='"+this.id+"'] option").attr('selected', false);
//$(this).parents(".graph-addmore").find(".submenu_addmore select option[value!='']").attr('selected', false);
//var optionText = $('#dropdownList option[value="'+optionValue+'"]').text();
var selected=$(this).val();
$('select#'+this.id+' option').attr('selected', false)

$('select#'+this.id+' option[value="'+selected+'"]').attr('selected', true);

});*/
 

$(document).on('touchstart','.nav_editab', function(e){
