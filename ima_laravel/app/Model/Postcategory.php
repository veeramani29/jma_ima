...
public function getCatDetails(){
	...
	$response=array();
	//$get_Cat = Postcategory::where('post_category_status', 'Y')-> where('post_category_parent_id','4')->get();	
	//$get_Arr = $get_Cat->toArray();
	$sql = "SELECT t2.* FROM `post_category` as t2 WHERE t2.`post_category_parent_id` IN (SELECT t1.`post_category_id` FROM `post_category` as t1 where t1.post_category_status = 'Y' AND t1.`post_category_parent_id` = 4)";
	$rs = DB::select($sql);
	$get_Count =count($rs);
	if($get_Count>0) {
	foreach($rs as $key=>$value){
		$response[$value->post_category_id] = $value->post_category_name;
	}
	}
	return $response;
}
...
public function getCatUrl($catId){
	...
	$response='';
	$sql = "SELECT `category_url` FROM post_category WHERE post_category_id = $catId AND `post_category_status` = 'Y'";
	$rs = DB::select($sql);
	$get_Count =count($rs);
	if($get_Count>0) {
	foreach($rs as $key=>$value){
		$response = $value->category_url;
	}
	}
	return $response;
}
...
public function updateNewIcon($catId,$status) {
		$response = false;
		try{
			if($status == 'Y' || $status == 'N') {	$rs =Postcategory::where('post_category_id',$catId)->update(array('new_icon_display'=>$status));
	...
	public function gettesttt($post_cat_id){
		$result_arr = array();

		 $get_Cat = DB::select("SELECT a.title,a.gid,a.updated_page,b.post_id,b.post_category_id FROM `graph_details` a,`post` b WHERE a.gid in (35,18,62,61,59,7,50,69,63,66,68,67,55) and a.updated_page=b.post_id ORDER by a.gid asc");
		 $get_Count =count($get_Cat);

		if($get_Count>0) {
			$get_Cat_ = json_decode(json_encode($get_Cat), true);
			foreach ($get_Cat_ as $get_Cats) {
				$Find_Category=$this->getAllParentCategoriesByCategoryId($get_Cats['post_category_id']);

			$Find_Parent_Category=array_filter($Find_Category, function($k) {
			return $k['post_category_parent_id'] == 0;
			});

			    #echo "<pre>";print_r($Find_Parent_Category);die;
			$result_arr[$Find_Parent_Category[0]['post_category_name']][] = array("gid"=>$get_Cats['gid'],"post_id"=>$get_Cats['post_id'],"title"=>$get_Cats['title'],"cat_id"=>$get_Cats['post_category_id'],"parent_cat_id"=>$Find_Parent_Category[0]['post_category_id'],"post_category_name"=>$Find_Parent_Category[0]['post_category_name']);
			}
