...
class User extends Model
{
	...
	function checkLinkedInUserExists($userdata,$password,$user_type_id,$user_type,$defultAlertValue=false){
        ...
        $country = $this->getCountryId($userdata->country_code);
        // $country ='';
   ...
   $check = User::where('email', $email)->get();
	...
	public function addCompetitionUserRegistration($competitionUserDetails)
	{
		if(is_array($competitionUserDetails))
		{
			$uid =  DB::table(TBL_COMPETITION_USER)->insertGetId($competitionUserDetails);
			
		    return $uid;
		}
	}

    public function updateCompetitionPPTX($userDetails) {
		$response = false;


        $rs=DB::table(TBL_COMPETITION_USER)->where('account_user_id',$userDetails['account_user_id'])->update($userDetails);
	...
	public function getCompetitionUserById($id) {
		$response = false;
        $rs=DB::table(TBL_COMPETITION_USER)->where('account_user_id',$id)->get();
        if(count($rs)>0) {
             $response = true;
        }
        return $response;
