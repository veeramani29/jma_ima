<?php
namespace App\Libraries\linkedIn;
use Exception;
use Session;
...
*/
	...
	/* Process for Linkedin Oauth2 version v2*/

	function ProcessV2()
	{
#dd($this->authorization_url);
			...
			'redirect_uri'=>$this->redirect_uri,
			'scope'=>$this->scope,
			'state'=> $_SESSION['oauth2state_Linkedin']
			);

			$url=$this->authorization_url;
			 $url.= (strpos($url, '?') === false ? '?' : '&').http_build_query($authorizationData);
			$this->Redirect($url);
			$this->exit = true;return true;
	}


function getAccessToken_($arPar)
	{
		...
		$parameters=array_merge($authorizationData,$arPar);
		#dd($parameters);
		$url=$this->access_token_url;$method='POST';
	$return=$this->send_req($url, $parameters, $method);
...
$this->authorization_error =$result['error'].':'.$result['error_description'];
}
...
public function CallLinkedinApiData($url,$method,$format) {
$curl = curl_init();
