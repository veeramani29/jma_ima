...
$getCopyWriter = $copywriterObj->getIndividualCopyWriter($editId);

if(count($getCopyWriter)>0){
    $insertArray['copywriter_user']  = $getCopyWriter[0]['copywriter_user'];
    $insertArray['copywriter_email'] = $getCopyWriter[0]['copywriter_email'];
}
 $insertArray = array();
  if(isset($_POST['copywriteEdit'])){
      
	if($_POST['copywriteEdit']){
		...
		if($email == ''){
			$errorMsg ='Please enter all mandatory field<br/>';
		        ...
		        $copywriterObj->updateWriter($editId,cleanInputField($email),cleanInputField($password));
...
<div id="page-heading"><h1>Edit Copy writer</h1></div>
	...
	<td>
		...
		<!-- start id-form -->
				...
				<div class="error_sent_notification"><?php echo $errorMsg;?></div>
	             <?php } ?>
                    ...
                    <tr>
                            ...
                            <td>
                                <input type="text"  disabled="disabled"  name="username" id="username" class="inp-form" <?php if (isset($insertArray['copywriter_user'])) { ?>value="<?php echo $insertArray['copywriter_user']; ?>"<?php } ?> />
                ...
                <tr>
                    <th>&nbsp;</th>
                    <td valign="top">
                            <input type="submit" value="Submit" name="copywriteEdit" class="form-submit" />
