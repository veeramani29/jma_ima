...
class category{
   ...
   */
   ...
   function updteCategories2($updateId,$category, $title, $keywords, $desc, $category_url, $category_url_key, $post_category_parent_id,$category_type,$category_link,$email_alert,$default_email_alert,$premium_category,$default_premium_email_alert){
            $this->db->open();
            ...
            if(count($chk_result)>0) {
            	...
            	$this->db->open();
	            $query_pag_data   = "update post_category set post_category_name  = '$category', category_meta_title='$title', category_meta_keywords='$keywords', category_meta_description='$desc', category_url='$category_url', category_url_key='$category_url_key', category_type = '$category_type', category_link = '$category_link' , email_alert = '$email_alert' , default_email_alert = '$default_email_alert', premium_category = '$premium_category' , default_premium_email_alert = '$default_premium_email_alert' where post_category_id = $updateId";
	           // exit($query_pag_data);
	            $results = $this->db->query($query_pag_data );
		    	$this->db->close();
	            return $results;
		...
		try{
			if($status == 'Y' || $status == 'N') {
				...
				if($this->db->query($sql)) {
					$response = true;
          $updatePostDatetime = "UPDATE post SET `post_datetime` = now() WHERE post_category_id = $catId";
          if($this->db->query($updatePostDatetime)) {
            $response = true;
          }
				}
			} else {
