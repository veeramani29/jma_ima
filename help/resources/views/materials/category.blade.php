...
<div class="col-xs-12 col-md-10">
	...
	else
	{
		...
		<div class="row">
			...
			<div class="col-xs-12 col-md-6">
				<img alt="Oxford Economics" src="<?php echo images_path("image_oxford_material.png");?>">
			</div>
		</div>
		<?php }?>
		<?php
		...
		foreach ($resn as $materials) {
			...
			<div class="row premat_con">
				...
				<div class="col-md-8">
					...
					<a class="btn btn-secondry" href="<?php echo url('materials/download/'.$id.'/'.$material_path);?>" target="_blank">
					...
					<?php }else{
						...
						// check permission
							...
							// show upgrade box
								...
								<div class="spacer10"></div>
								<a class="btn btn-secondry <?php echo $premium_cat_lnk_cls;?>" href="javascript:void(0)" onclick="JMA.User.showUpgradeBox('premium','<?php echo url('materials/download/'.$id.'/'.$material_path);?>')" class="<?php echo $premium_cat_lnk_cls;?>">
									<i class="fa fa-download"></i> <?php echo $material_path;?>
								</a>
								<?php }
							}else{
							// Show login window
								$premium_cat_lnk_cls = 'lnk_inactive';?>
								<div class="spacer10"></div>
								<a class="btn btn-secondry <?php echo $premium_cat_lnk_cls;?>" href="javascript:void(0)" onclick="JMA.User.showLoginBox('premium','<?php echo url('materials/download/'.$id.'/'.$material_path);?>')" class="<?php echo $premium_cat_lnk_cls;?>">
									<i class="fa fa-download"></i> <?php echo $material_path;?>
								</a>
								<?php
							}
					...
					<p class="matinjap_con">
						<a href="<?php echo url('materials/download/'.$id.'/'.$material_path);?>" target="_blank"><?php echo $material_path;?></a>
					</p>
					<?php }else{
						$premium_cat_lnk_cls = '';
						if($result['login_status']==true) {
